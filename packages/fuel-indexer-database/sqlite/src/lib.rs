...
pub async fn foreign_key_list_by_name(
    ...
    let query = format!("SELECT db_type, namespace, table_name, column_name, reference_table_name, reference_column_name, reference_column_type, on_delete, on_update FROM graph_registry_foreign_keys WHERE schema_version = {} AND schema_name = {}", schema_version, schema_name);
    ...
    for row in rows {
        let table_name = row.get(3);
        let column_name = row.get(4);
        let reference_table_name = row.get(5);
        let reference_column_name = row.get(6);
        let reference_column_type = row.get(7);
        let raw_db_type = row.get(8);
        let namespace = row.get(9);
        let raw_on_delete = row.get(10);
        let raw_on_update = row.get(11);
